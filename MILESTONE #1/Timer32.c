...
// we divide period by 2 because we want an interrupt for 
...
void MS_Timeout_Handler(void) { timeOut = TRUE; }
...
void Timer32_1_Init(void(*task)(void), unsigned long period, enum timer32divider div) {
	...
	// bit7       (1),       timer 0=disable, 1=enable 
	// bit6       (1),       0 = Timer is in free-running mode, 1=timer in periodic mode
	// bit5       (-),       interrupt 0=disable, 1=enable
	// bit4       (X),       reserved
	// bits3-2    (-),       input clock divider according to parameter
	// bit1       (1),       0=16bit counter, 1=32-bit counter
...
void Timer32_2_Init(void(*task)(void), unsigned long period, enum timer32divider div) {
	...
	// timer reload value
	// TIMER32_LOAD2
	TIMER32_2 -> LOAD = timer2Period;;    

	// clear Timer32 Timer 2 interrupt
	// TIMER32_INTCLR2
	TIMER32_2 -> INTCLR &= 0x01;
