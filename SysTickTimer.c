...
void EnableSysTickTimer(void) {
	// enable SysTick with core clock and interrupts (SYSTICK_STCSR)
...
void DisableSysTickTimer(void) {
	// disable SysTick with core clock and interrupts (SYSTICK_STCSR)
...
void SysTickTimer_Init(void(*task)(void), unsigned long period) {
	...
	// 1) disable SysTick with core clock and interrupts
	DisableSysTickTimer();
	
	// NOTE: The STRVR - RELOAD VALUE REGISTER)is a 24 bit value
	// 2) reload value sets period (SYSTICK_STRVR)
	SYSTICK_STRVR = period;  
	
	// Current Value Register
	// 3) any write to current clears it (SYSTICK_STCVR)
	SYSTICK_STCVR = 1;
